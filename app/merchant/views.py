...
def listing_view_all(page=1):
    ...
    paginated = Listing.return_paginator(available=True,
                                         favorite=favorite,
                                         name_search_term=name_search_term,
                                         main_search_term=main_search_term).paginate(page,20,False)
...
def add_to_cart():
    ...
    print int(current_listing_id)
    page=int(int(current_listing_id)/20)
    db.session.commit()
    return redirect(url_for('merchant.listing_view_all', page=page))
